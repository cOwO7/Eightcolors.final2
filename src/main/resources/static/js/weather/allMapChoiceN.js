...
document.addEventListener("DOMContentLoaded", function () {
	...
	if (navigator.geolocation) {
		navigator.geolocation.getCurrentPosition(function (position) {
			...
			console.warn("현재 위치를 가져올 수 없습니다. 기본 위치로 설정합니다.");
		});
	}

	// 주소 검색 버튼 클릭 시
	addressBtn?.addEventListener("click", openDaumPostcode);
	...
	function updateCoordinates(lat, lng) {
		const gridCoordinates = dfs_xy_conv("toXY", lat, lng);
	...
	// 지역 코드 업데이트
	function updateRegionCodes(address) {
		const [city, district] = address.split(" ");
		if (!city || !district) {
			console.error("주소에서 시/도와 구/군 정보를 추출할 수 없습니다:", address);
			return;
		}
		...
		regIdInput.value = cityData.reg_code.regId || "Unknown";
		regIdTempInput.value = cityData.reg_code.regIdTemp || "Unknown";
	...
	function fetchCoordinates(city, district) {
		...
		fetch(`/coordinates/${korCode}`)
			...
			.then(data => {
				const { gridX: nx, gridY: ny, latitude, longitude } = data;
				document.getElementById("nx").value = nx;
				document.getElementById("ny").value = ny;
				document.getElementById("latitudeNum").value = latitude;
				document.getElementById("longitudeNum").value = longitude;
			})
